...
import { RegistryStorage } from "core/tenderize-v3/registry/RegistryStorage.sol";
import { FACTORY_ROLE, FEE_GAUGE_ROLE, TENDERIZER_ROLE, UPGRADE_ROLE, GOVERNANCE_ROLE } from "core/tenderize-v3/registry/Roles.sol";
...
contract Registry is Initializable, UUPSUpgradeable, AccessControlUpgradeable, RegistryStorage {
    ...
    error TenderizerAlreadyExists(address asset, bytes32 validator, address tenderizer);

    event AdapterRegistered(address indexed asset, address indexed adapter);
    event NewTenderizer(address indexed asset, bytes32 indexed validator, address tenderizer);
     ...
     */
    function getTenderizer(address asset, bytes32 validator) external view returns (address) {
     ...
     * @param tenderizer Address of the tenderizer
     */
    function registerTenderizer(address asset, bytes32 validator, address tenderizer) external onlyRole(FACTORY_ROLE) {
        Storage storage $ = _loadStorage();
