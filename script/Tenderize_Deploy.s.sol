...
contract Tenderize_Deploy is Script {
    function run() public {
        ...
        address registryProxy = address(new ERC1967Proxy(address(registry), ""));
        vm.serializeAddress(json_output, "registry_proxy", registryProxy);

        // 2. Deploy Tenderizer Implementation
        Tenderizer tenderizer = new Tenderizer();
        ...
        ERC1967Proxy rendererProxy = new ERC1967Proxy(address(renderer), abi.encodeCall(renderer.initialize, ()));
        vm.serializeAddress(json_output, "renderer_proxy", address(rendererProxy));
        // - Deploy Unlocks
        Unlocks unlocks = new Unlocks(address(rendererProxy), registryProxy);
        ...
        // 5. Deploy Factory
        Factory factory = new Factory(address(registryProxy));
